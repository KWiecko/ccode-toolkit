...
Answer the **same done-questionnaire** the worker fills (canonical list in `worker.md`) — independently: Feature · Built (what you can see was built) · **Real-usage test (Directive 3)** · Why it works (or doesn't) · Weak spots · Requirements (each → met? + evidence) · Open/uncertain. Then flag, per item, where your answer would **diverge** from a confident self-assessment — divergence is the signal. **Scrutinize the Real-usage-test item hardest:** was it exercised the way it's *actually* used (UI → rendered output at a real terminal + a human smoke; long-running → past limits / over time), or only via a mock/state-check blind to render/scroll/wrap/rollover/over-time bugs? A green state-check is where the worst bugs hide.
